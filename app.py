...
def main():
    ...
    def load_data():
        data = pd.read_csv("herg_features.tsv", sep="\t")
        data.columns = data.iloc[0]
        data=data.drop(data.index[0])
        data=data.drop(['WT', 'E4', 'temp'], axis=1)
    ...
    def split(df):
        #y = df.pathogenic
        #x = df.drop(columns=['WT', 'E4', 'temp','pathogenic'])
        features=["Hydrophobicity_Change", "Distance_to_SF(A)", "Conservative_Score",]
        output = ["is class 2"]
        x = df[features]
        y = df[output]
    ...
    st.markdown('<a href="mailto:ikalyanc@gmail.com">Created by Kalyan Immadisetty</a>',unsafe_allow_html=True)  
